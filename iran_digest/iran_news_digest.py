...
GROQ_PROMPT = """You are a senior analyst specializing in Iranian affairs.
...
def summarize_with_groq(articles: list[dict]) -> str:
    ...
    articles_text = "\n\n".join(article_lines)
    prompt = GROQ_PROMPT.format(articles=articles_text)
...
def _parse_digest(digest: str) -> list[dict]:
    """
    Parse Groq output into a list of story dicts:
